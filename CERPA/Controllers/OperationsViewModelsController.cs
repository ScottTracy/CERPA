...
namespace CERPA.Controllers
{
    public class OperationsViewModelsController : Controller
    {
        ...
        public async Task<ActionResult> Index()
        {
            ...
            StartJobAssignments(order);

            return View(await db.OperationsViewModels.ToListAsync());
        }
        ...
        public async void CreateJob(Job job)
        {
            if (ModelState.IsValid)
            {
                db.Jobs.Add(job);
        ...
        public void StartJobAssignments(Order order)
        {
            ...
            foreach (var partStructure in partStructures)
            {
                ...
                CreateQuantityValue(partquantity);
                var pickOrder = AssignPickOrder(job, partStructure.ChildID, partquantity.ChildQuantityValue);
                CreatePickOrder(pickOrder);
                AssignChildJobs(partStructure, order, partquantity.ChildQuantityValue);
            }
        }
        public void AssignChildJobs(PartStructure partStructure, Order order, int quantity)
        {
